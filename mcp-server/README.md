...
### Canonical Adstream tools (preferred)

Use these tools for new prompts and integrations. They keep the public MCP surface provider-agnostic and avoid report-specific tool proliferation.

| Tool | Purpose |
|---|---|
| `ads_list_accounts` | List accessible ads accounts for Meta/TikTok/Google-capable broker mode |
| `ads_list_campaigns` | List campaigns for an account |
| `ads_get_performance` | Fetch normalized ads performance with `provider`, `accountId`, `since`, `until`, `level`, `metrics`, `dimensions`, `breakdowns`, `filters`, `limit`, and `cursor` |
| `ads_get_creatives` | Fetch creative-level metadata and metrics using the same canonical envelope |
| `ads_get_change_history` | Fetch Meta account activities in a canonical change-history envelope; unsupported providers return structured `NOT_IMPLEMENTED` |
| `ads_get_capabilities` | Discover canonical tools, provider capabilities, metric catalogs, breakdowns, and optional writes |
| `commerce_get_performance` | Fetch commerce/SKU/product/order performance with a canonical commerce envelope |

The `meta_*` and `tiktok_*` tools below remain available for backward compatibility. Prefer canonical tools for new work. Daily reports, weekly reports, audits, recommendations, KPI scoring, and top/bottom rankings should be generated by the AI or skills after fetching data, not by adding new MCP report tools.

### Legacy provider-specific tools
