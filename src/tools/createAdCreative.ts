...
import { resolvePartnershipIdentityFromAdCode } from './listPartnershipContent.js';
...
import {
  ...
  type AssetFeedSpecCreateVerdict,
...
export const DYNAMIC_CREATIVE_DISABLED_MESSAGE =
  'asset_feed_spec dengan optimization_type REGULAR (atau tanpa optimization_type sama sekali — Meta mengisinya REGULAR) adalah jalur Dynamic Creative, dan itu dinonaktifkan di MCP ini karena Meta mewajibkan ad set is_dynamic_creative=true, ad set harus kosong, dan hanya boleh berisi satu ad. Yang menentukan adalah optimization_type, bukan jumlah aset. Untuk beberapa varian primary text/headline pada ad BIASA, kirim optimization_type "DEGREES_OF_FREEDOM" (Advantage+ text variations — tidak butuh ad set dynamic dan boleh banyak ad per ad set). Untuk aset berbeda per placement/language/segment, pakai asset_customization_rules (minimal 2). Alternatif lain: beberapa manual creative/ad terpisah, atau carousel untuk multi-card media.';
...
export async function createAdCreative(
  ...
  try {
    ...
    const enrichedOptions = await withAutoVideoThumbnail(client, partnershipReadyOptions);
...
export { classifyAssetFeedSpecForCreate };
...
function validationError(message: string): StructuredMutationError {
  ...
  if (message.includes(DYNAMIC_CREATIVE_DISABLED_MESSAGE)) {
    return {
      ...
      message,
      actionableFix:
        'Set asset_feed_spec.optimization_type ke "DEGREES_OF_FREEDOM" bila yang Anda mau adalah beberapa varian primary text/headline pada ad biasa (Advantage+ text variations). Untuk aset berbeda per placement/language/segment, tambahkan asset_customization_rules (minimal 2). Atau gunakan creativeFormat single_image, video, carousel, existing_post, catalog, collection, dan buat beberapa manual creative/ad terpisah.',
    };
  }
  return {
    provider: 'meta',
