...
const cpasCatalogBundleInputSchema = {
  ...
  resumeFrom: z
    ...
    .strict()
    .optional()
    .describe('Resume a partial bundle without recreating successful parent objects.'),
...
const placementCustomizedAssetFeedSpecSchema = z.record(z.unknown()).refine(
  (value) => {
    return assetFeedSpecCreateError(value) === undefined;
  },
  {
    message:
      'asset_feed_spec optimization_type REGULAR atau kosong adalah Dynamic Creative dan ditolak. Untuk beberapa primary text/headline pada ad biasa, set optimization_type "DEGREES_OF_FREEDOM". Untuk placement/language/asset customization, sertakan asset_customization_rules minimal 2.',
  }
);

const objectStorySpecInputSchema = z
  .object({
    asset_feed_spec: placementCustomizedAssetFeedSpecSchema.optional(),
  })
  .passthrough()
  .describe(
    'Input advanced/backward-compatible Meta object_story_spec. Untuk video/link asset_feed_spec, tetap isi link_data.link selain asset_feed_spec.link_urls. asset_feed_spec bersarang boleh untuk Advantage+ text variations dengan optimization_type="DEGREES_OF_FREEDOM", atau asset customization dengan asset_customization_rules minimal 2. Dynamic Creative tetap ditolak: optimization_type REGULAR atau kosong.'
  );
...
export const createAdCreativeInputSchema = {
  ...
  creativeSpec: z
    ...
    .optional()
    .describe(
      'Detail materi sesuai creativeFormat. Jika user memberi opsi headline/caption/copy/image/video untuk testing manual dan ingin varian terukur terpisah, buat beberapa manual creative/ad dengan creativeFormat single_image/video/carousel dan satu media + satu primaryText + satu headline per creative, atau pakai carousel cards untuk beberapa media dalam satu carousel. Jika ingin Meta memilih beberapa primary text/headline pada satu ad biasa, gunakan assetFeedSpec.optimization_type="DEGREES_OF_FREEDOM" dengan bodies/titles; untuk video/link asset-feed, tetap isi objectStorySpec.link_data.link. Untuk placement/language/asset customization, gunakan asset_customization_rules minimal 2. Dynamic Creative tetap ditolak: optimization_type REGULAR atau kosong. Field per format: single_image memakai imageHash, primaryText, destinationUrl, headline, description, callToAction, pageWelcomeMessage (opsional, untuk Click-to-WhatsApp/Messenger), dan applinkTreatment (opsional, enum: deeplink_with_appstore_fallback, deeplink_with_web_fallback, web_only, deeplink_disabled — hanya berlaku saat collaborativeAppSpec diisi, default automatic; pada mode: collaborative_ads field ini diabaikan untuk video/single_image); video memakai videoId, thumbnailImageHash (opsional — kalau kosong, otomatis diisi dari thumbnail bawaan video via GET /{videoId}?fields=picture; hanya berbahaya diabaikan kalau video belum selesai diproses Meta dan tidak punya thumbnail sama sekali), primaryText, destinationUrl, headline, description, callToAction, pageWelcomeMessage (opsional, untuk Click-to-WhatsApp/Messenger), dan applinkTreatment (opsional, sama seperti single_image); carousel memakai primaryText, destinationUrl, cards (imageHash atau videoId, headline, description, destinationUrl); catalog memakai productSetId, primaryText, destinationUrl, templateUrl legacy, fallbackImageHash, presentation, hybridVideo, showMultipleImages, preferredImageTags, formatOption, dan categorizationCriteria; collection memakai instantExperienceId, coverImageHash atau coverVideoId, productSetId, primaryText, destinationUrl; placement_image memakai asset_feed_spec khusus placement; placement_customized_ctwa memakai link_data utama, platform_customizations, portrait_customizations, dan pageWelcomeMessage di link_data; existing_post memakai objectStoryId (post id Facebook Page, format {page_id}_{post_id}) ATAU sourceInstagramMediaId (media id IG yang tidak di-cross-post ke Page — dapatkan dari ads_list_instagram_media, cocokkan permalink-nya ke URL instagram.com/reel atau /p yang dimiliki user; wajib isi tepat satu dari dua field ini, KECUALI pada jalur partnership ad code (partnership.adCode) yang tidak memakai keduanya karena ad code sendiri yang menjadi referensi konten; untuk media VIDEO/Reel WAJIB juga mengisi instagramUserId tingkat atas, kalau tidak Meta menolak dengan (#100) subcode 1815279 yang keliru menyuruh mengunggah video ke Facebook — tidak perlu diunggah, Meta hanya tidak tahu akun IG pemiliknya; media IMAGE disimpulkan sendiri oleh Meta sehingga tidak butuh field itu), plus destinationUrl, callToAction, pageWelcomeMessage (opsional hanya untuk Messenger/Instagram Direct), dan applinkTreatment (opsional). CTWA/WHATSAPP_MESSAGE dengan existing_post DITOLAK karena Meta dapat menyimpan CTA di creative tanpa merender tombol pada ad; gunakan single_image atau video inline dengan media, body, identity, CTA, dan welcome message yang sama. Untuk mengarahkan post yang di-boost ke landing page eksternal dengan tombol CTA: isi destinationUrl + callToAction (mis. LEARN_MORE). Keduanya dikirim sebagai call_to_action di LEVEL ATAS creative (call_to_action.value.link), BUKAN di dalam object_story_spec — object_story_spec bareng source_instagram_media_id ditolak Meta dengan (#100) subcode 1487929 Ambiguous Promoted Object (terverifikasi live di v25.0). Tujuan post Instagram bisa diganti bebas; post Facebook Page yang sudah punya link sendiri mungkin tetap memakai link lamanya — nilainya diteruskan dan Meta yang memutuskan. Pakai urlTags untuk tracking UTM; itu tersimpan bersama call_to_action. destinationUrl juga wajib diisi kalau collaborativeAppSpec diisi, dipakai untuk omnichannel_link_spec.web.url (CATATAN: itu pun tidak bisa memperbaiki object_store_urls yang hilang dari call_to_action post lama yang sudah dipublikasikan; untuk ad set CPAS omnichannel disarankan pakai creativeFormat video langsung). destinationUrl tanpa callToAction maupun collaborativeAppSpec akan DITOLAK, bukan diabaikan diam-diam. Untuk iklan click-to-message Messenger/Instagram Direct pada existing_post: isi callToAction MESSAGE_PAGE atau INSTAGRAM_MESSAGE, appDestination yang sesuai, dan destinationUrl (untuk Instagram Direct gunakan https://www.instagram.com/). Kombinasi appDestination + destinationUrl dikirim sebagai call_to_action.value.app_destination dan call_to_action.value.link; Meta Graph menolak appDestination tanpa link untuk existing-post Instagram messaging. destinationUrl dengan CTA messaging tetapi tanpa appDestination tetap DITOLAK agar URL tidak ter-drop diam-diam. Welcome message DIDUKUNG pada existing_post hanya untuk Messenger/Instagram Direct; CTWA wajib memakai creative inline single_image atau video. Penempatannya di LEVEL ATAS creative — bentuk yang didokumentasikan Meta (page_welcome_message di dalam object_story_spec) DITOLAK untuk existing_post karena object_story_spec tidak boleh berbarengan dengan source_instagram_media_id; terverifikasi live di v25.0 (2026-08-24). Jangan simpulkan boost post tidak bisa punya welcome message hanya karena contoh resmi Meta untuk konten Instagram tidak memuat field ini. pageWelcomeMessage boleh berupa objek page_welcome_message VISUAL_EDITOR penuh ({ type, version, landing_screen_type, media_type, text_format.message.ice_breakers }) atau string; untuk existing_post + callToAction WHATSAPP_MESSAGE, string sapaan akan di-normalize otomatis menjadi objek VISUAL_EDITOR ctwa_call_prompt agar create ad tidak ditolak Meta dengan code 2. Dikirim sebagai page_welcome_message di LEVEL ATAS creative, persis seperti yang ditulis Ads Manager, dan hanya berlaku bersama callToAction messaging. Field creativeSpec di luar daftar per format di atas DITOLAK dengan error yang menyebut field-nya, bukan dibuang diam-diam.'
    ),
  ...
  welcomeMessageTemplateName: z
    ...
    .optional()
    .describe(
      'Nama reusable welcome message template lokal dari ads_list_welcome_message_templates. Saat diisi, template dikembangkan menjadi creativeSpec.pageWelcomeMessage sebelum creative dibuat. Jangan isi bersamaan dengan creativeSpec.pageWelcomeMessage. Berlaku untuk creativeFormat yang mendukung pageWelcomeMessage; pada existing_post hanya Messenger/Instagram Direct yang didukung karena CTWA wajib memakai single_image atau video inline.'
    ),
  instagramUserId: z.string().optional().describe('Instagram user ID for IG posting.'),
  threadsProfileId: z.string().optional().describe('Threads profile ID for Threads posting.'),
  ...
  whatsappWelcomeMessageSequenceId: z
    ...
    .optional()
    .describe(
      'ID Meta WhatsApp welcome-message sequence. Dikirim sebagai asset_feed_spec.additional_data.partner_app_welcome_message_flow_id. Tidak didukung untuk existing_post karena CTWA wajib memakai single_image atau video inline. Jangan gabungkan dengan pageWelcomeMessage atau welcomeMessageTemplateName.'
    ),
  dedupeByName: z
    .boolean()
    .optional()
