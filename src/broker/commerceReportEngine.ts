...
const DEFAULT_COMMERCE_DISCLAIMER =
  'These commerce recommendations are suggestions only. Review actual order quality, cancellations, fees, and margins before taking action.';
...
function calculateTotals(records: CommerceRecord[]): CommerceReportTotals {
  ...
  if (adSpend !== undefined && adSpend > 0)
    totals.roas_commerce = roundMetric(totals.gmv / adSpend);
...
function buildFindings(totals: CommerceReportTotals, recordCount: number): string[] {
  ...
  if (totals.roas_commerce !== undefined) {
    findings.push(
      `Commerce ROAS was ${formatNumber(totals.roas_commerce)} based on GMV and ad spend.`
    );
  }
...
function buildRecommendations(totals: CommerceReportTotals, recordCount: number): string[] {
  if (recordCount === 0)
    return ['Confirm store IDs, date range, and GMV Max access before drawing conclusions.'];
  ...
  if (totals.roas_commerce !== undefined) {
    return [
      'Review product feed, offer competitiveness, and conversion quality before scaling GMV Max spend.',
    ];
  }
