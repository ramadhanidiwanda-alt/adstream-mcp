...
4. Saat memakai Ad Set lama, mencampur manual/static creative dengan flexible asset-feed, catalog/product, atau placement-customized asset-feed sebaiknya dihindari, tapi tidak diblokir. `ads_create_ad` dry-run hanya memberi *warning* untuk creative-family mismatch dan tetap melanjutkan — Ad Set non-Dynamic Creative (`is_dynamic_creative=false`) umumnya menerimanya. Yang benar-benar ditolak adalah Ad Set `is_dynamic_creative=true` (Meta mensyaratkan Ad Set kosong dan hanya satu ad). Kalau Meta akhirnya menolak, baru rekomendasikan duplikat/buat Ad Set baru; `skipAdSetCreativeFamilyCheck: true` mematikan advisory-nya.
