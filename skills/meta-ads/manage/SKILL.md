...
4. Ad Set lama boleh berisi campuran format creative — manual/static, Advantage+ text variations (`optimization_type: DEGREES_OF_FREEDOM`), dan asset customization bisa berdampingan. Tidak ada aturan Meta yang melarangnya. Satu-satunya yang diblokir adalah Dynamic Creative (`optimization_type: REGULAR`, termasuk saat field itu tidak dikirim karena Meta mengisinya REGULAR), yang oleh Meta diwajibkan sendirian di Ad Set `is_dynamic_creative=true`. Kalau maksudnya beberapa varian primary text/headline pada ad biasa, kirim `optimization_type: "DEGREES_OF_FREEDOM"`.
