...
3. Jika marketer meminta beberapa opsi headline/caption/copy, ada dua jalur sah — pilih berdasarkan siapa yang memilih aset. Kalau mau Meta yang memilih varian teks pada satu ad biasa (Advantage+ text variations), kirim `assetFeedSpec` dengan `optimization_type: "DEGREES_OF_FREEDOM"`. Kalau mau menguji varian secara terpisah dan terukur, buat beberapa manual creative/ad terpisah, masing-masing satu media, satu `primaryText`, dan satu `headline`; atau carousel cards bila formatnya carousel. Untuk aset berbeda per placement/language/segment, pakai `asset_customization_rules` (minimal 2). Yang disabled hanya `optimization_type: "REGULAR"` — atau `assetFeedSpec` tanpa `optimization_type` sama sekali, karena Meta mengisinya `REGULAR` — yaitu jalur Dynamic Creative. Jangan memakai `creativeFormat: "flexible"`, `primaryTexts`, `headlines`, atau `isDynamicCreative`.
4. Ad Set lama boleh berisi campuran format creative — manual/static, Advantage+ text variations (`optimization_type: DEGREES_OF_FREEDOM`), asset customization, dan katalog bisa berdampingan. Tidak ada aturan Meta yang melarangnya. `ads_create_ad` dry-run hanya memberi _catatan informasi_ menyebutkan keluarga apa saja yang sudah ada di Ad Set itu, lalu tetap melanjutkan; `skipAdSetCreativeFamilyCheck: true` mematikan catatan itu beserta Graph read-nya. Yang benar-benar ditolak adalah Dynamic Creative (`optimization_type: REGULAR`, termasuk saat field itu tidak dikirim karena Meta mengisinya REGULAR), yang oleh Meta diwajibkan sendirian di Ad Set `is_dynamic_creative=true` ("your ad set must be empty ... You can only create one ad per ad set"). Kalau maksudnya beberapa varian primary text/headline pada ad biasa, kirim `optimization_type: "DEGREES_OF_FREEDOM"`.
