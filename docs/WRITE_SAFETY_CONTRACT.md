...
| `ads_create_ad`                       | Ad                | Omnichannel, messaging destination, Dynamic Creative asset feeds classified by `optimization_type`; creative-family mismatch is advisory-only | —                                                                                                            |
...
- **Dynamic Creative asset feeds:** Before `ads_create_ad`, classify the creative's `asset_feed_spec` by its `optimization_type`, not by asset count and not by the mere presence of `asset_customization_rules`. Reject only `REGULAR` — including a spec that omits `optimization_type`, since Meta then stores `REGULAR` — because that is Dynamic Creative, which Meta requires to sit alone in an `is_dynamic_creative=true` ad set ("your ad set must be empty ... You can only create one ad per ad set"). `DEGREES_OF_FREEDOM` (Advantage+ text variations), rule-based asset customization, and `FORMAT_AUTOMATION` are ordinary ads and MUST be allowed.
- **Ad creative-family mismatch (advisory, never a block):** Before `ads_create_ad`, read the target Ad Set's existing non-archived ads and note when the new creative's family differs from what the Ad Set already holds. Informational only. **No Meta rule forbids mixing creative formats in one Ad Set** — a production Ad Set (`is_dynamic_creative:false`) was observed running four `DEGREES_OF_FREEDOM` ads alongside a manual/static ad, and live creates on 2026-08-18 confirmed the same. Classify families by `optimization_type` so Advantage+ text variations are not mislabelled Dynamic Creative in the note. `skipAdSetCreativeFamilyCheck: true` suppresses the advisory and its Graph reads.

> **Do not reintroduce:** the earlier version of that check _blocked_ the create and cited Meta error `#1885274` as the reason. That subcode appears in no Meta error reference and no other source, and no Meta documentation forbids mixing creative formats in one Ad Set. Never cite it again.

Pre-flight errors MUST be returned BEFORE any Meta API call with an actionable message.

**Pre-flight results MUST NOT be reported as Meta rejections.** A local pre-flight is a prediction, not an API verdict. `ads_create_ad` returns `status: "preflight_blocked"` with `errorSource: "local_preflight"` and the `preflightCheck` name; only an error actually returned by the Graph API may use `status: "failed"` with `errorSource: "meta_api"`. Never phrase a pre-flight message as "Meta rejected/menolak".
