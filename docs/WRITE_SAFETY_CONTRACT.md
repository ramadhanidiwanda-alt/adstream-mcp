...
| `ads_create_ad`                       | Ad                | Ad set/creative compatibility (omnichannel, messaging destination, and Dynamic Creative asset feeds classified by `optimization_type`) | —                                                                                                            |
...
- **Dynamic Creative asset feeds:** Before `ads_create_ad`, classify the creative's `asset_feed_spec` by its `optimization_type`, not by asset count and not by the mere presence of `asset_customization_rules`. Reject only `REGULAR` — including a spec that omits `optimization_type`, since Meta then stores `REGULAR` — because that is Dynamic Creative, which Meta requires to sit alone in an `is_dynamic_creative=true` ad set. `DEGREES_OF_FREEDOM` (Advantage+ text variations) and rule-based asset customization are ordinary ads and MUST be allowed.

> **Removed — do not reintroduce:** an "ad creative-family mismatch" pre-flight that rejected mixing manual/static creatives with asset-feed creatives in one Ad Set, citing Meta error `#1885274`. That subcode appears in no Meta error reference and no other source, no Meta documentation forbids mixing creative formats in one Ad Set, and a production Ad Set (`is_dynamic_creative:false`) was observed running four `DEGREES_OF_FREEDOM` ads alongside a manual/static ad. `skipAdSetCreativeFamilyCheck` remains accepted as a no-op that only emits a warning.
