...
Use the same four tools for either mode: create the campaign with `ads_create_campaign`, create one ad set with `ads_create_adset`, create each format separately with `ads_create_adcreative`, then connect each creative to that ad set with `ads_create_ad`. One `adsetId` can be reused freely across formats — manual/static, Advantage+ text variations, asset-customized, and catalog ads coexist in the same Ad Set, and no Meta rule forbids mixing them. The `ads_create_ad` dry-run reads the target Ad Set and returns an informational _warning_ naming the families already there; set `skipAdSetCreativeFamilyCheck: true` to skip the advisory and its Graph reads. The one real Ad Set constraint belongs to Dynamic Creative — `asset_feed_spec` with `optimization_type: REGULAR`, which is also what Meta stores when you omit the field: Meta requires such an ad to sit alone in an `is_dynamic_creative=true` Ad Set, and that path stays rejected outright. Send `optimization_type: "DEGREES_OF_FREEDOM"` when you want several primary text/headline variants on an ordinary ad. A `collection` creative must reuse an existing `instantExperienceId`—the connector does not build Instant Experience content.
...
Meta's catalog ads also auto-generate video from product images by default (`media_type_automation`, `enroll_status: OPT_IN`) since October 2025 — no code or extra field is needed here to get it; every catalog creative already created through this connector has it live-verified as active. To opt out for a specific creative, pass `optOutEnhancements: ['media_type_automation']`; catalog creatives now emit the same typed `degrees_of_freedom_spec` opt-out used by the other supported creative formats.

`ads_create_cpas_catalog_bundle` also accepts typed `campaignSettings`, `adSetSettings`, and `creativeSettings` overrides. These expose catalog-safe campaign categories/buying settings; bid, schedule, attribution, audience, placement, DSA, and multi-advertiser controls; plus `showMultipleImages`, image tags, format/category options, URL tags, and enhancement opt-outs. Unknown nested fields are rejected instead of being forwarded as raw Graph API payloads.
