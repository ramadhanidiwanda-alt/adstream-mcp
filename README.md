...
Use the same four tools for either mode: create the campaign with `ads_create_campaign`, create one ad set with `ads_create_adset`, create each format separately with `ads_create_adcreative`, then connect each creative to that ad set with `ads_create_ad`. One `adsetId` can be reused by compatible manual formats such as image and video ads. Mixing manual/static ads with flexible asset-feed, catalog/product, or placement-customized asset-feed ads in the same Ad Set is discouraged but not blocked: the `ads_create_ad` dry-run reads the target Ad Set's existing ads and returns a *warning* on a creative-family mismatch, leaving the decision to Meta. Meta's own hard constraint is the Ad Set flag `is_dynamic_creative=true` (the Ad Set must be empty and may hold only one ad), and that path stays rejected outright. Set `skipAdSetCreativeFamilyCheck: true` to skip the advisory. A `collection` creative must reuse an existing `instantExperienceId`—the connector does not build Instant Experience content.
